...
f1 = path.resolve('%CD%', './plugins/demarrage/item/plugins.json').replace('\\%CD%', '');
...
ScribeSpeak("je vérifie dans les xml" , function() {
longueur1=(nameList.length)
//on lis la liste des phrases(item)
for (g=0;g<longueur5;g++){
  f2=filePath = path.resolve('%CD%', './plugins/demarrage/item/'+objet5.nompluguine[g] +'item.json').replace('\\%CD%', '');
data=fs.readFileSync(f2,'utf8').toString(); 
...
ScribeSpeak("rien dans les xml",function(){
...
lo='';pos=str.indexOf(toSearch)
  ...
  //  console.log(pos);console.log(lon);console.log('rrrrrrrrr')
    if(lo.indexOf('Build')>-1){console.log('trouver');//callback({'tts' : " "})
    ;return false}; 
...
setTimeout(function(){ 
  console.log(url1)
   request = require('request');
request({ url : url1 }, function (err, response, body){
ScribeSpeak("Pluguine "+nomduplug);//callback({'tts' : " "});
return false
});//fin request
return false
; }, 1000);//fin timeout
...
if (query.search(new RegExp("\\b" + " traduction de " + "\\b","gi")) >-1){
  query22 = query.search("traduction de");//console.log('query22'+query22);
  query23 = query.length;//console.log('query23'+query23);
  for (i = (query22+13); i < query23 ;i++){reponse=reponse+(query[i]);//console.log('reponse'+reponse);
  }
 ScribeSpeak("mots clés trouvé traduction",function(){
SARAH.run('traduction', { 'chercher' : reponse});//callback({'tts' : " "});
})
return false
  }

if (query1.search("traduction") >-1){
...
if (query.search("site") >-1){
  query22 = query.search("site");//console.log('query22'+query22);
  query23 = query.length;//console.log('query23'+query23);
  for (i = (query22+5); i < query23 ;i++){reponse=reponse+(query[i]);//console.log('reponse'+reponse);
  }
 ScribeSpeak("mots clés trouvé site",function(){
SARAH.run('mathildeurl', { 'urlchercher' : reponse});//callback({'tts' : " "});
})
return false
  }
...
if (query1.search("synonymes") >-1){
 ...
 ScribeSpeak("mots clés trouvé synomyme",function(){console.log('************'+reponse)
...
if (query1.search("synonyme") >-1){
 ...
 ScribeSpeak("mots clés trouvé synonyme",function(){console.log('!!!!!!!!!!!!!!!!!!'+reponse)
SARAH.run('cyrano', { 'chercher' : reponse});//callback({'tts' : " "});
})
return false
  }
...
if (query1.search("vidéos") >-1){
 ...
 ScribeSpeak("mots clés trouvé video",function(){
...
if (query1.search("vidéo") >-1){
 ...
 ScribeSpeak("mots clés trouvé vidéo",function(){
   process1 = '%CD%/plugins/cortana/bin/searchyoutube.vbs ' + reponse ; exec(process1)})
   return false
//callback({'tts' : " "}); 
}
...
if (query1.search("images") >-1){ 
  ...
  query23 = query1.length;
  for (i = (query22+6); i < query23 ;i++){reponse=reponse+(query1[i]);}
     ScribeSpeak("mots clés trouvé image",function(){
...
//var answer1 = 'Voici le raisultat de votre recherche sur ' + reponse ;
 if (reponse==""){reponse="false"}
   ScribeSpeak("mots clés trouvé courses",function(){
...
//var answer1 = 'Voici le raisultat de votre recherche sur ' + reponse ;
 ...
 ScribeSpeak("mots clés trouvé mémo",function(){
...
//var answer1 = 'Voici le raisultat de votre recherche sur ' + reponse ;
 if (reponse==""){reponse="false"}
  ScribeSpeak("mots clés trouvé météo",function(){
...
//var answer1 = 'Voici le raisultat de votre recherche sur ' + reponse ;
 ...
 ScribeSpeak("mots clés trouvé horoscope",function(){
 SARAH.run('horoscopemathilde', { 'phrasehoroscope' : query});//callback({'tts' : " "});
 })
 return false
  }
...
if(reponse==""){//console.log('***************************riennnnnnnnnnnnnn')
  //ask(query)
  ScribeSpeak("je n'ai pas trouvé de mots clés",function(){
  match3(query)
  reponse="eee"
})//fin speak
...
//if (nameList[j].search(new RegExp("\\b" + nameListe[j] + "\\b","gi"))>-1){
  ...
  if (j==longueur2-1){console.log('trouver'); emulate1(motsplugin,nomduplug);//callback({'tts' : " "})
