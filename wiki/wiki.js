...
j=1;k=0;
 ...
 request = require('request');
 cheerio = require('../modules/cheerio');

////////////////////////////////////////////////////////////////////////
function encart(phrase1){
//request('https://www.google.fr/search?hl=fr&q='+phrase1+'&gws_rd=ssl', function (error, response, html) {
request({'uri':'https://www.google.fr/search?hl=fr&q='+phrase1+'&gws_rd=ssl', 'headers':{'Accept-Charset': 'utf-8'}, 'encoding':'binary' }, function (error, response, html) {
//request({ 'uri' : url, 'headers':{'Accept-Charset': 'utf-8'}, 'encoding':'binary' }, function(error, response, html){
    var $ = cheerio.load(html);
 
$('span:nth-child(1)._tA').each(function(i, element){var a = $(this);console.log('trounvé '+a.text())
if (a.text()!==""){ScribeSpeak(a.text(),function(){return false})}
//if (fin=='on'){console.log('fini')}
})//fin for each
...
//////////////////////////////////////////////////////////////////////////
req = function(url){ 
ScribeSpeak("je ne connais pas",function(){
 ...
 if (err || response.statusCode != 200) {console.log(err)
  //return callback({'tts': "Action échouée"})
   ScribeSpeak('rien dans wikipedia',function(){
...
mafonction1=function(){paragaphe5=paragaphe1.length+paragaphe2.length+paragaphe3.length+paragaphe4.length+paragaphe6.length
  if (paragaphe5==0){
    ScribeSpeak('rien dans wikipedia',function(){
      process1 = '%CD%/plugins/cortana/bin/search.vbs ' + search;exec(process1);return false
      })
  }//fin if
else{ ScribeSpeak(paragaphe1+" "+paragaphe2+" "+paragaphe3+" "+paragaphe4+" "+paragaphe6,true,function(){
WikiMemoire(search);return false
}) }
} //fin mafnct1
//callback();
